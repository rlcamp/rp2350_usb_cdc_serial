...
int main(void) {
    ...
    while (1) {
        ...
        void * staging_area = usb_cdc_serial_in_staging_area();
        const void * out_buf = usb_cdc_serial_out_staging_area();
        ...
        for (size_t ipass = 0; ipass < 2 && get_dtr(); ipass++) {
            usb_cdc_serial_in_start_sending(sizeof(wherry) - 1);

            while (get_dtr() && usb_cdc_serial_in_still_sending())
                yield();
        }

        while (get_dtr()) {
            const size_t bytes_to_echo = usb_cdc_serial_out_filled();

            if (bytes_to_echo && !usb_cdc_serial_in_still_sending()) {
                unaligned_memcpy(staging_area, out_buf, bytes_to_echo);

                usb_cdc_serial_in_start_sending(bytes_to_echo);
                while (__dsb(), get_dtr() && usb_cdc_serial_in_still_sending())
                    yield();
                usb_cdc_serial_out_rearm();
